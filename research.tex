...
and Tur\'{a}n stated a conjecture in \cite{Erdos:1936fk} on a
generalization of van der Waerden's Theorem has been the source for a
vibrant avenue of research.
To state their conjecture we define the \textsl{upper asymptotic
  ...
  \[
    \overline{d}(A) = \limsup_{N\to\infty} \frac{|A \cap \{1, 2,
      \ldots, N\}|}{N}.
  \]
Erd\H{o}s and Tur\'{a}n conjectured that if $\overline{d}(A) > 0$,
then $A$ contains a $k$-term arithmetic progression for every natural
number $k$. 

Their conjecture turned out to be both correct and a hard problem; it
was proved almost forty years later by Szemer\'{e}di in
...
complicated, combinatorial argument. 
Shortly after this original proof, Furstenburg give another proof
in \cite{Furstenberg:1977fk} that used Ergodic Theory. 
After these initial proofs, a number of other different proofs have since
been discovered.
We summarize these difference methods in the following table.
Limited space and mathematical competence prohibits us from accurately
and sufficiently explaining how each method of proof works. 
Hence we can do little more than name-drop in our table. 
However, we shall soon see that even this name-dropping will serve a
purpose. 

  \begin{tabular}[center]{lcl}
    \toprule
    Author(s) & Citation & What (and how) they proved\\
    \midrule
    Furstenburg and Katznelzon & \cite{Furstenburg:1989kx},
    \cite{Furstenburg:1991vn} & Density Hales-Jewett Theorem (Ergodic
    Theory) \\
    Furstenburg and Katznelzon & \cite{Furstenburg:1978ys} & Multidimensional Szemer\'{e}di
    Theorem (Ergodic Theory) \\
    Gowers & \cite{Gowers:2001uq} & Szemer\'{e}di's Theorem
    (finite Fourier analysis argument)
  \end{tabular}
